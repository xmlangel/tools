...
IMPORTANT: The output must be strictly in {target_lang}.

**제목** – 8~15자, 핵심 키워드만 사용하고 “~에 대한 이야기”와 같은 완구적인 표현은 피한다.  
**한줄소개** – 50~200자, 핵심 인사이트와 “요약해보면” 같은 서두는 사용하지 않는다.  
**핵심 키워드** – 3~5개의 핵심 단어, 쉼표(,)로 구분.  
**주요 내용** – 4~7개의 핵심 포인트를 각각 200~500자 이내의 문장으로 정리.  
**TL;DR** – 200~5000자 이내, 전체 내용의 핵심을 한 문단에 압축.  

**작성 가이드**  
- 모든 항목은 반드시 **{target_lang}**로만 작성해야 합니다. (Must be written in {target_lang})
...
이 텍스트를 기반으로 위 시스템 프롬프트에 명시된 규칙과 형식에 맞춰 **{target_lang}**로 요약을 작성해 주세요.
(Please write the summary in {target_lang})
...
def get_template():
    ...
    # JSON 파일이 없으면 DEFAULT_TEMPLATE을 기반으로 생성
    logger.info(f"Template file not found. Creating {TEMPLATE_FILE} with DEFAULT_TEMPLATE")
